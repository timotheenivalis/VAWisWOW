...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\hspace{-1cm}
		...
		\parbox{24.5cm}{\textsc{How to model fitness?}\\
		  \centering  \begin{tabular}{c c}
		    \includegraphics[width=20cm]{Rgraph/figure/zidist-1.pdf}
		    \end{tabular}
		    \footnotesize Zero-Inflated Over-Dispersed Poisson models tend to fit well lifetime fitness data. 
		  }
		&

		&
		\parbox{24.5cm}{ \hspace{2cm} \textsc{How to estimate genetic variation?}\\
		   {\centering \begin{tabular}{c} 
		   \end{tabular}}
		   
		   \footnotesize Quantitative genetic \textit{animal models} from relatedness matrix. Estimate G-matrix for the zero-inflation and the over-dispersed Poisson.
		  }
		&
		  
		&
		\parbox{24.5cm}{\textsc{How to convert estimates to $V_A(\omega)$}\\
		{\centering \begin{tabular}{c}
		  \includegraphics[width=20cm]{Rgraph/VaPopGrowth.pdf}
		\end{tabular}} 
				
		\footnotesize Monte-Carlo integration of multivariate latent breeding values to the scale of fitness. Back-transformation matches theoretical expectation: with simulations $V_A(\omega)$ equals the increase in population growth rate.
	      }
		\end{tabular}
		\vspace{-1cm}
	}%end block
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
 ...
 %% the content of the block
          ...
          \node[anchor=north] (map) at ($(box.north)+(0,-3)$) {\includegraphics[width=20cm]{Rgraph/map.pdf}};
          ...
          \node[anchor=south east] (ziva) at (box.south east) {\includegraphics[width=20cm]{Rgraph/zih2.pdf}};
...
{\color{colorone}{\**} Fisher's theorem relies on stringent assumptions, or alternatively on quite a specific meaning of evolution:  XXXXXXXXXXXXXX XXX XXXXXXXXXXXXX. In the absence of competition and gene-by-environment interactions, $V_A(\omega)$ is also the increase in expected population growth rate. Such a clean net between evolution and demography probably never exist in nature, or not for long anyways.}
  }
