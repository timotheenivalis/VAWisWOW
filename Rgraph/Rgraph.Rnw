...
<<zidist, dev='tikz'>>=
barplot(rbind(drho, dpczi[1:14], dpc[1:14]), beside = TRUE, col = c("black", "purple", "orange"), las=1, ylab = "density", border = c("black", "purple", "orange"), xlab="Lifetime reproductive success")
lines(y = density(dg)$y, x=density(dg)$x*4, col=rgb(1,0,0, 0.8), lwd=3)
legend(x = "topright", legend = c("Observed data", "ZI Poisson prediction", "Poisson prediction", "Gaussian prediction"), fill = c("black", "purple", "orange", NA), lwd = c(NA, NA, NA, 3), lty = c(NA, NA, NA, 1), box.lwd = c(1,1,1,0), col = c("black", "purple", "orange", rgb(1,0,0, 0.8)),border = c(NA))
@
